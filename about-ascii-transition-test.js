(() => {
  ...
  const MODE = document.body.dataset.asciiTest || 'scramble';
  ...
  (async () => {
    ...
    const prep = () => {
      ...
      for (let i = 0; i < cells; i += 1) {
        const start = hash(seed + i * 19) * 0.54;
        const duration = 0.26 + hash(seed + i * 41) * 0.22;
        starts[i] = start;
    ...
    const paletteChar = value => {
      const index = Math.max(0, Math.min(PALETTE.length - 1, Math.round((value / 255) * (PALETTE.length - 1))));
      return PALETTE[index];
    };

    const scrambledChar = (seed, cycle) =>
      SCRAMBLE_POOL[Math.floor(hash(seed + cycle * 911) * SCRAMBLE_POOL.length) % SCRAMBLE_POOL.length];

    const charForScatteredScramble = (i, morph, tick, seed, currentFrame, nextFrame) => {
      const start = starts[i];
      const end = ends[i];
      if (morph <= start) return paletteChar(currentFrame[i]);
      if (morph >= end) return paletteChar(nextFrame[i]);

      const local = clamp((morph - start) / Math.max(0.001, end - start));
      const cycle = Math.min(SCRAMBLE_CYCLES - 1, Math.floor(local * SCRAMBLE_CYCLES));
      return scrambledChar(seed + i * 47 + tick * 131, cycle);
    };

    const diagonalPosition = i => {
      const x = i % COLS;
      const y = Math.floor(i / COLS);
      return ((x / Math.max(1, COLS - 1)) + (y / Math.max(1, ROWS - 1))) * 0.5;
    };

    const charForWave = (i, morph, tick, seed, currentFrame, nextFrame, hybrid) => {
      const diagonal = diagonalPosition(i);
      const center = morph * (1 + WAVE_BAND * 2) - WAVE_BAND;
      const local = clamp((center + WAVE_BAND - diagonal) / (WAVE_BAND * 2));

      if (local <= 0) return paletteChar(currentFrame[i]);
      if (local >= 1) return paletteChar(nextFrame[i]);

      if (hybrid) {
        const cycle = Math.min(SCRAMBLE_CYCLES - 1, Math.floor(local * SCRAMBLE_CYCLES));
        return scrambledChar(seed + i * 61, cycle);
      }

      return scrambledChar(seed + i * 71 + tick * 197, 0);
    };
    ...
    const draw = (now, morph = 0) => {
      ...
      const tick = Math.floor(now / GLYPH_MS);
      const seed = (current + 1) * 1063 + (next + 1) * 2207 + serial * 131;

      for (let i = 0; i < cells; i += 1) {
        let char;
        if (morph <= 0) {
          char = paletteChar(currentFrame[i]);
        } else if (MODE === 'wave') {
          char = charForWave(i, morph, tick, seed, currentFrame, nextFrame, false);
        } else if (MODE === 'hybrid') {
          char = charForWave(i, morph, tick, seed, currentFrame, nextFrame, true);
        } else {
          char = charForScatteredScramble(i, morph, tick, seed, currentFrame, nextFrame);
        }
        lines[Math.floor(i / COLS)][i % COLS] = char;
      }
    ...
    scrollState();
    hero.dataset.asciiFrame = String(current + 1);
    hero.dataset.asciiState = `test-${MODE}`;
    ...
    console.error('About ASCII transition test failed.', error);
  });
})();

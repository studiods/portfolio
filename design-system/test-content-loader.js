...
const data=main.querySelector('#data');const dataWrap=data?.querySelector(':scope > .hm-wrap');const dataHead=dataWrap?.querySelector(':scope > .hm-section-head');if(dataHead){const title=dataHead.querySelector('.hm-section-title'),desc=dataHead.querySelector('.hm-section-desc');if(title)title.innerHTML='그리고 실제로 고객들이 서비스를 어떻게 이용하고 있는지도 살펴봤습니다.';if(desc)desc.innerHTML='고객의 말만으로 문제를 단정하지 않았습니다. 유입, 랜딩, 검색, 상품 상세, 장바구니, 결제까지 실제 행동을 따라가며 <strong>어디에서 다음 행동이 약해지는지</strong>를 확인했습니다.';
...
const journey=main.querySelector('#journey');const journeyWrap=journey?.querySelector(':scope > .hm-wrap');const journeyHead=journeyWrap?.querySelector(':scope > .hm-section-head');if(journeyHead){const title=journeyHead.querySelector('.hm-section-title'),desc=journeyHead.querySelector('.hm-section-desc');if(title)title.innerHTML='그래서 화면보다 먼저,<br>여정을 설계하는 세 가지 원칙을 정했습니다.';if(desc)desc.innerHTML='자주 방문하지 않는 가전 쇼핑의 특성과 하이마트의 오프라인 자산을 함께 놓고, 이후 모든 화면 설계가 따라야 할 기준을 먼저 만들었습니다.';const roleSections=[...journeyWrap.querySelectorAll(':scope > .hm-subsection')];if(roleSections.length)wrapMore(journeyWrap,roleSections,'여정별 역할 정의 자세히 보기','ROLE DEFINITION')}

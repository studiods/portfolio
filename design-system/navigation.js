...
/*
  TEST-ONLY wide editorial baseline synchronizer.
  The alignment reference is the first ACTUAL visible content line in the right rail,
  not the top of its wrapper. This means authored margin/padding inside .hm-subsection,
  .data-card or .flow-area is naturally accounted for instead of being mistaken for
  the content baseline.
*/
(() => {
  'use strict';

  const mountWideEditorialBaseline = () => {
    if (!document.body.classList.contains('hm-wide-editorial-test')) return;
    if (window.__hmWideEditorialBaselineMounted) return;
    window.__hmWideEditorialBaselineMounted = true;

    const wideQuery = window.matchMedia('(min-width:1600px)');
    const sections = [...document.querySelectorAll('#live-main > :is(#brand,#data,#journey,#direction).hm-section')];
    if (!sections.length) return;

    const rightAnchorSelectors = {
      brand: '.hm-subsection .hm-subhead > .hm-subno',
      data: '.data-list .data-card .data-card-head > .hm-card-no',
      journey: '.hm-subsection .hm-subhead > .hm-subno',
      direction: '.flow-area .flow-label, .flow-area .flow-node .hm-card-no'
    };
    ...
    const sync = () => {
      ...
      sections.forEach(section => {
        const head = section.querySelector('.hm-section-head');
        if (!head) return;
        head.style.setProperty('--hm-wide-baseline-offset', '0px');
      });

      if (!wideQuery.matches) return;

      /* Force one layout pass after resetting offsets so measurements use authored geometry. */
      void document.documentElement.offsetHeight;

      sections.forEach(section => {
        const head = section.querySelector('.hm-section-head');
        const leftAnchor = head?.querySelector('.hm-section-no');
        const selector = rightAnchorSelectors[section.id];
        const rightAnchor = selector ? section.querySelector(selector) : null;
        if (!head || !leftAnchor || !rightAnchor) return;

        const delta = layoutTop(rightAnchor) - layoutTop(leftAnchor);
        if (!Number.isFinite(delta)) return;
        head.style.setProperty('--hm-wide-baseline-offset', `${delta.toFixed(2)}px`);
      });
    };
    ...
    wideQuery.addEventListener?.('change', requestSync);
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', mountWideEditorialBaseline, {once:true});
  } else {
    mountWideEditorialBaseline();
  }
})();
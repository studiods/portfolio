/* HIMART — restrained inertial / damped wheel scrolling.
   Uses the real window scroll position (no transform wrapper) so sticky hero,
   IntersectionObserver, progress nav and scroll-driven hero fade keep their native geometry.
   Desktop fine-pointer only; touch/coarse pointers and reduced-motion users stay native. */
(() => {
  'use strict';

  const body = document.body;
  if (!body || !body.classList.contains('hm-wide-himart-test')) return;
  ...
  const onWheel = event => {
    ...
    if (!raf) currentY = window.scrollY || window.pageYOffset || 0;
  ...
  const onScroll = () => {
    ...
    /* Our own rAF scroll writes should not cancel the easing. External jumps such as
       scrollbar dragging, keyboard/page navigation or anchor navigation should. */

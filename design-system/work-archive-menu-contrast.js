(()=>{
  ...
  const surfaceLuminance=()=>{
    ...
    const x=clamp(shellRect.left+Math.min(shellRect.width*.5,200),8,innerWidth-8);
    const y=open&&menuRect.height>20
      ? clamp(menuRect.top+Math.min(menuRect.height*.46,260),8,innerHeight-8)
      : clamp(shellRect.top+Math.min(shellRect.height*.5,20),8,innerHeight-8);
    const elements=underlyingElements(x,y);
    const image=findImage(elements,x,y);
    const imageLum=sampleImage(image,x,y);
  ...
  const update=()=>{
    frame=0;
    const lum=surfaceLuminance();
    shell.dataset.waSurface=lum>.58?'light':'dark';
  };
  const schedule=()=>{
    if(frame)return;

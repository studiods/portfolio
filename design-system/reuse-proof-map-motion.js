(() => {
  ...
  // One lane = ring sweep -> connector draw -> lower-circle fill.
  // The next lane starts when the previous lane reaches 30% of its 2.5s runtime.
  const LANE_DURATION = 2500;
  const STAGGER = LANE_DURATION * 0.30; // 750ms
  ...
  const runLane = (index) => {
    ...
    // 0.0s -> 1.5s: the white segment leaves 6 o'clock, travels clockwise,
    // and returns to 6 o'clock while the passed line immediately dims again.
    anxiety.classList.add(RING_CLASS);

    // 1.5s -> 2.1s: as soon as the sweep returns, draw the hand-off downward.
    schedule(() => {
    ...
    // 2.1s -> 2.5s: when the connector reaches the lower circle, fill it blue
    // and invert its title to black.
    schedule(() => {
      if (!active || !inFocus) return;
    ...
    // Kept as an explicit boundary so the full lane is exactly 2.5 seconds.
    schedule(() => {
      if (!active || !inFocus) return;
      connector.classList.add(CONNECTOR_HELD_CLASS);
      solution.classList.add(SOLUTION_FILLED_CLASS);

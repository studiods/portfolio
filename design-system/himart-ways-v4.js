(() => {
  ...
  /*
    ...
    fix is CSS ownership, not another DOM MutationObserver. Load the Ways-only reset as
    the final stylesheet layer and never rewrite/clone/repair the 03 title in JavaScript.
  */
  const mountTitleOwnershipCss = () => {
    if (document.querySelector('link[data-ways-title-owner]')) return;
    const link = document.createElement('link');
    link.rel = 'stylesheet';
    link.href = './design-system/components/himart-ways-title-owner.css?v=20260913-1';
    link.dataset.waysTitleOwner = '1';

(()=>{
'use strict';
const main=document.getElementById('himart-main')||document.getElementById('test-main');
if(!main)return;
const SOURCE_BLOB='6824a76b15854f4608498951bd3f909373cc407b';
const loadSource=()=>fetch(`https://api.github.com/repos/studiods/portfolio/git/blobs/${SOURCE_BLOB}`,{cache:'no-store'})
  .then(r=>{if(!r.ok)throw new Error('source load failed');return r.json()})
  .then(j=>{const bin=atob(j.content.replace(/\s/g,''));const bytes=Uint8Array.from(bin,c=>c.charCodeAt(0));return new TextDecoder('utf-8').decode(bytes)});
...
try{
  ...
  if(!sourceMain)throw new Error('main not found');
  ...
  const summary=subByNo(main.querySelector('#brand'),'01.3');if(summary)setText(summary.querySelector('.hm-subcopy'),'긍정은 전문성과 오프라인 자산에, 부정은 그 강점이 온라인에서 끊기는 순간에 집중됐습니다.');
  setList('#brand .conclusion-card p',['설치·A/S·매장 체험·전문가 상담처럼 실패 가능성을 낮추는 경험이 선택의 핵심 이유였습니다.','앱 속도·복잡한 혜택·배송·설치 불확실성과 정보 차이가 전문성에 대한 기대를 불신으로 바꿨습니다.']);
  const uxPoints=subByNo(main.querySelector('#brand'),'01.4');if(uxPoints){setText(uxPoints.querySelector('.hm-subcopy'),'고객이 반복해서 말한 기대와 불편을 이후 설계에서 놓치지 않도록 핵심 원칙으로 정리했습니다.');setList('.direction-card p',['매장 상담의 신뢰를 온라인에서도 이어가고, 필요한 순간 AI Agent와 데이터 가이드가 개입하도록 확장합니다.','설치·보증·A/S·관리·중고판매를 한 흐름으로 연결해 구매 이후 관계가 이어지도록 설계합니다.','온라인의 관심·비교 맥락이 매장까지 이어지고, 매장 경험이 다시 온라인 구매로 돌아오게 합니다.','추천검색·동적 필터·핵심 정보 위계와 AI 요약으로 탐색 비용을 줄이고 후보를 빠르게 압축합니다.','총액·혜택·설치 가능일·진행 상태를 명확히 보여줘 다시 문의하지 않아도 현재 상태를 이해하게 합니다.'],uxPoints);}
  ...
  const reveal=[...document.querySelectorAll('.hm-reveal')];if(reduced)reveal.forEach(n=>n.classList.add('is-in'));else{const rio=new IntersectionObserver(es=>es.forEach(e=>{if(e.isIntersecting){e.target.classList.add('is-in');rio.unobserve(e.target)}}),{threshold:.1,rootMargin:'0px 0px -8% 0px'});reveal.forEach(n=>rio.observe(n));}
  const sections=[...document.querySelectorAll('[data-chapter]')],links=[...document.querySelectorAll('.hm-progress a')];const update=()=>{let active=0,best=1e9;sections.forEach((s,i)=>{const d=Math.abs(s.getBoundingClientRect().top-innerHeight*.33);if(d<best){best=d;active=i}});links.forEach((l,i)=>l.classList.toggle('is-active',i===active))};addEventListener('scroll',()=>requestAnimationFrame(update),{passive:true});update();
  ...
  const flowScript=document.createElement('script');flowScript.src='./himart-flow-line-sync-v1.js?v=ca60638';document.body.appendChild(flowScript);
}catch(err){main.innerHTML='<div class="hm-wrap" style="padding-top:180px;min-height:70vh"><h1 style="font-size:42px;font-weight:100">페이지를 불러오지 못했습니다.</h1><p style="color:rgba(255,255,255,.5)">잠시 후 다시 시도해 주세요.</p></div>';console.error(err);}
})();
})();
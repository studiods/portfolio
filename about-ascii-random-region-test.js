(() => {
  ...
  /* Keep the production cadence exactly the same. */
  const SCENE_MS = 1500;
  const HOLD_MS = 960;
  const MORPH_MS = 540;
  const GLITCH_MS = 48;
  ...
  /*
    Test idea:
    - preserve the current value interpolation / palette / glitch language
    - remove any directional wipe or explicit scramble layer
    - correlate nearby cells with low-frequency spatial noise, so multiple
      small regions begin changing at different random moments
    - add only a small per-cell jitter so regions do not look blocky
  */
  const COARSE_X = 10;
  const COARSE_Y = 6;
  const FINE_X = 22;
  const FINE_Y = 13;
  const START_SPREAD = 0.56;
  const CELL_JITTER = 0.055;
  const MIN_DURATION = 0.28;
  const DURATION_RANGE = 0.22;
  ...
  (async () => {
    ...
    const lattice = (x, y, gridX, gridY, seed) => {
      const gx = clamp(x) * gridX;
      const gy = clamp(y) * gridY;
      const x0 = Math.floor(gx);
      const y0 = Math.floor(gy);
      const x1 = Math.min(gridX, x0 + 1);
      const y1 = Math.min(gridY, y0 + 1);
      const tx = smooth(gx - x0);
      const ty = smooth(gy - y0);
      const stride = gridX + 1;
      const value = (ix, iy) => hash(seed + iy * stride * 131 + ix * 977);
      const a = value(x0, y0) * (1 - tx) + value(x1, y0) * tx;
      const b = value(x0, y1) * (1 - tx) + value(x1, y1) * tx;
      return a * (1 - ty) + b * ty;
    };
    ...
    const prep = () => {
      ...
      for (let i = 0; i < cells; i += 1) {
        ...
        const coarse = lattice(x, y, COARSE_X, COARSE_Y, seed + 101);
        const fine = lattice(x, y, FINE_X, FINE_Y, seed + 1709);
        const jitter = (hash(seed + i * 43) - 0.5) * CELL_JITTER;

        /* Low-frequency regions lead; fine noise only softens their edges. */
        const regionTiming = coarse * 0.74 + fine * 0.26;
        const start = clamp(regionTiming * START_SPREAD + jitter, 0, 0.64);
        const duration = MIN_DURATION + hash(seed + i * 79) * DURATION_RANGE;
    ...
    scrollState();
    hero.dataset.asciiFrame = String(current + 1);
    hero.dataset.asciiState = 'test-random-regions';
    ...
    console.error('About random-region ASCII test failed.', error);
  });
})();

...
const revealAtMiddle=()=>{if(!brand)return;brand.querySelectorAll('.hm-reveal:not(.is-in)').forEach(el=>{const r=el.getBoundingClientRect();if(r.top<=innerHeight*.6&&r.bottom>=innerHeight*.4)el.classList.add('is-in');});};
let raf=0;

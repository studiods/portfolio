...
let raf=0;
const update=()=>{raf=0;if(!stage||!fixed)return;const progress=clamp(window.scrollY/Math.max(1,innerHeight));const blackout=clamp(.85*(1-Math.pow(1-progress,3)));fixed.style.setProperty('--hm-video-blackout',blackout.toFixed(4));fixed.style.setProperty('--hm-frame-opacity',(1-blackout).toFixed(4));revealAtMiddle();};
